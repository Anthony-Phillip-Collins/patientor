...
const HealthCheckFields = ({
  ...
  const healthRatingKeys = Object.keys(HealthCheckRating).filter((key) =>
    isNaN(Number(key))
  );
  const healthRatingValues = Object.keys(HealthCheckRating).filter(
    (key) => !isNaN(Number(key))
  );
  const getKeyFromValue = (value: string): string => {
    const indx = Number(value);
    return healthRatingKeys[
      Math.min(healthRatingKeys.length - 1, Math.max(0, indx))
    ];
  };
  const getRating = () =>
    healthRatingValues.find(
      (value) => Number(value) === Number(healthCheckRating)
    );
